...
target_shape = [1,112,112,12]

inputs = tf.keras.Input(shape=input_shape[1:],batch_size=input_shape[0])
outputs = tf.keras.layers.Reshape(target_shape)(inputs)
model = tf.keras.Model(inputs=inputs, outputs=outputs)
# Print summary.
model.summary()
# Save model and weights in a h5 file, then load again using tf.keras.
model.save('reshape.h5')
model = tf.keras.models.load_model('reshape.h5', compile=False)
...
with tf.io.gfile.GFile('reshape.tflite', 'wb') as f:
